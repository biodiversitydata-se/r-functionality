#' Scatterplot of environmental parameters associated with species occurrence data. Interactive plotly plot of environmental variables and species occurence data
#' 
#' Makes a search using \code{\link{occurrences}}and plots environmental variables associated with species presence against each other. Alternatively, makes a search using \code{\link{occurrences}}and plots species occurence data e.g frequencies against environmental variables. Note that there is a limit of 500000 records per request when using \code{method="indexed"}. Use the \code{method="offline"} for larger requests. For small requests, \code{method="indexed"} likely to be faster. 
#' 
#' @references \itemize{
